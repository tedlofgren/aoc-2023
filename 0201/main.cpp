...
int main(int num_args, char** args)
{
	...
	uint32_t sum_of_possible_game_ids = 0;

	constexpr uint32_t MAX_RED_CUBES = 12u;
	constexpr uint32_t MAX_GREEN_CUBES = 13u;
	constexpr uint32_t MAX_BLUE_CUBES = 14u;
	...
	while (true) {
		...
		while (true) {
			...
			while (true) {
				...
				is_game_possible = red <= MAX_RED_CUBES && green <= MAX_GREEN_CUBES && blue <= MAX_BLUE_CUBES;
				if (!is_game_possible)
					break;

				if (comma_separated_list[0] == ',')
	...
	printf("Q: What is the sum of the IDs of those games?\nA: %d", sum_of_possible_game_ids);
